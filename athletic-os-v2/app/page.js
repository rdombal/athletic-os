...
function MoveScreen({ onSave }) {
  ...
  return (
    <div>
      <div style={{ display:'flex', gap:8, padding:'12px 20px 0', borderBottom:'0.5px solid var(--border)' }}>
        {[['mobility','Mobility & relief'],['sport','Sport routines']].map(([k,l])=>(
          <button key={k} onClick={()=>setMode(k)} style={{
            padding:'8px 16px', borderRadius:'8px 8px 0 0', fontSize:13, border:'none',
            background:mode===k?'var(--surface)':'transparent',
            color:mode===k?'var(--text)':'var(--text3)',
            fontWeight:mode===k?500:400, borderBottom:mode===k?'2px solid var(--text)':'none',
          }}>{l}</button>
        ))}
      </div>
...
function MoreScreen({ onNav }) {
  ...
  return (
    <div style={{ padding:'20px 20px' }}>
      {items.map(item=>(
        <div key={item.tab} onClick={()=>onNav(item.tab)}
          style={{ background:'var(--surface)', border:'0.5px solid var(--border)', borderRadius:'12px',
            padding:'14px 16px', marginBottom:8, display:'flex', justifyContent:'space-between',
